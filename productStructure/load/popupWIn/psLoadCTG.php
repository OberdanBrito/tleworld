...
$sql_C = "SELECT englishName FROM codedefinitions
        WHERE type='Class' AND englishName!='Style' AND englishName!='Service' ORDER BY englishName ASC";
...
if (!EMPTY($num_rows_C)) {
    while ($data_C = mysql_fetch_array($dbquery_C)) {
        //----------------------------------------  TYPE  ----------------------------------------//
        $sql_T = "SELECT type,englishName FROM codedefinitions
                WHERE type='" . $data_C['englishName'] . "' ORDER BY englishName ASC";
        ...
        if (!EMPTY($num_rows_T)) {
            while ($data_T = mysql_fetch_array($dbquery_T)) {
                ...
                $sql_G = "SELECT englishName FROM codedefinitions
                        WHERE type='" . $data_T['englishName'] . "' ORDER BY englishName ASC";
                ...
                if (!EMPTY($num_rows_G)) {
                    while ($data_G = mysql_fetch_array($dbquery_G)) {
                        $Array_CTG .= $data_T["type"] . ":" . $data_T['englishName'] . ":" . $data_G['englishName'] . ",";
                    }
                } else {
                    $Array_CTG .= $data_T["type"] . ":" . $data_T['englishName'] . ",";
                }
            }
        } else {
            $Array_CTG .= $data_C['englishName'] . ",";
        }
    }
}
