...
<html >
    ...
    <body>
        ...
        <script>
            ...
            function changeValue(rId,cInd,nValue){
                if(cInd == obj.categoryEnglish){
                    var _type= nValue;
                    dhtmlxAjax.get("PHP/ajax/ajaxDetails2.php?type="+_type+"&lang="+lang+"&compare=all",function(loader){
                        var value = JSON.parse(loader.xmlDoc.responseText);
                        for(var i in value){
                            var code = value[i].split(":");
                            detGrid.getCombo(obj.value).put(code[1],code[0]);
                        }
                    });
                }
            }
            ...
            var data =[
                ...
                {fields:'value',aligns:'center',widths:'100',types:'coro',hides:false,enNames:'Value',thNames:'ข้อมูล'},
            ...
            detGrid.attachEvent("onCellChanged", changeValue);
            detGrid.init();
            ...
            if(getState == "edit"){
                ...
                detGrid.attachEvent("onXLE", function (grid_obj,count){    //when load data!!
                    if(detGrid.getAllRowIds() !== ""){
                        //don't  empty
                        var ar = detGrid.getAllRowIds().split(",");
                        for(var p in ar){
                            var _type2 = detGrid.cells(ar[p],obj.categoryEnglish).getValue();
                            var _code2 = detGrid.cells(ar[p],obj.value).getValue();
                            //                            if(_type2 == "payment Term"){
                            //                                alert(_type2+" : "+_value2+" : "+lang);
                            dhtmlxAjax.get("PHP/ajax/ajaxDetails2.php?type="+_type2+"&code="+_code2+"&lang="+lang+"&compare=one",function(loader){
                                var val = loader.xmlDoc.responseText;
                                //                                    console.dir(loader.xmlDoc);
                                detGrid.cells(ar[p],obj.value).setValue(val);
                            });
                            //                            }
                        }
                    }
