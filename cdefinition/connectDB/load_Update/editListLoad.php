...
//---------- Edit Data(below) ----------
$field = "cd.rowId,cd.englishName,cd.thaiName,cd.description,cd.type,cd.code,cd.groupCode,cd.startDate,cd.validUntil";
$tb = "codedefinitions";

//$cdtype = iconv("TIS-620", "UTF-8", $_REQUEST['cdtype'])
if($_REQUEST["cdtype"] != ""){
	$setWhere = "WHERE cd.type = '".addslashes($_REQUEST["cdtype"])."' ";
}else{
	$setWhere = "WHERE cd.type IS NULL ";
}
//----------  ----------
...
function getDataGroup( $gField, $gTb, $gArrayField, $gSetWhere) {
    if(!ISSET($gStrF)) {
        $sql = "SELECT $gField FROM $gTb AS cd $gSetWhere ";
    }
	//print($sql);
